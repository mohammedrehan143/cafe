  ...
  return (
    <>
      <AnimatePresence mode="wait">
        {trackingModalOpen && (
          <div
            key="order-tracking-portal-wrapper"
          ...
          >
            ...
            {/* Modal Content */}
            <motion.div
              key="tracking-modal-content-card"
            ...
            <div className="px-6 sm:px-8 py-5 border-b border-cream-300 bg-banhmi-card/90 flex items-center justify-between sticky top-0 z-20 backdrop-blur-md">
              <div>
                <span className="text-[10px] font-mono tracking-[0.25em] uppercase text-emerald-700 font-bold flex items-center space-x-1">
                  ...
                  {isCompleted ? 'Order Completed & Fulfilled' : 'Live Realtime Dispatch'}
                </span>
              ...
              <button
                onClick={() => setTrackingModalOpen(false)}
                className="p-2 rounded-full text-banhmi-dark/60 hover:text-banhmi-dark hover:bg-cream-200 transition-colors cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              {/* IF ORDER IS COMPLETED -> SHOW ONLY THANK YOU ANIMATION, RATING & FEEDBACK (NO OTP, NO TRACKING) */}
              {isCompleted ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="p-4 rounded-2xl bg-white border border-banhmi-red/30 shadow-sm flex flex-col justify-between">
                      ...
                      <button
                        ...
                        className="mt-2.5 px-3 py-1.5 rounded-lg bg-rose-50 hover:bg-rose-100 border border-rose-200 text-xs font-mono font-bold text-banhmi-red flex items-center justify-center space-x-1.5 transition-colors self-start cursor-pointer"
                      >
                    ...
                    <div className="p-4 rounded-2xl bg-white border border-cream-300 shadow-sm flex flex-col justify-between">
                      ...
                      <div className="mt-2.5 flex items-center gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 rounded-lg bg-cream-100 hover:bg-cream-200 border border-cream-300 text-xs font-mono font-bold text-banhmi-dark flex items-center space-x-1.5 transition-colors cursor-pointer"
                        >
                  ...
                  {currentOrder.customer?.deliveryInstructions && (
                    <div className="p-3.5 rounded-2xl bg-amber-50 border border-amber-200 text-xs font-mono space-y-1">
                      <div className="flex items-center space-x-1.5 font-bold text-amber-900">
                        <ChefHat className="w-3.5 h-3.5 text-amber-800" />
                        <span>Your Message for the Chef:</span>
                      </div>
                      <p className="text-amber-950 font-sans italic text-xs pl-5">
                  ...
                  <div className="p-4 rounded-2xl bg-white border border-[#4A2818]/20 shadow-xs space-y-2.5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        <div className="w-8 h-8 rounded-full bg-[#4A2818] text-white flex items-center justify-center">
                          <ChefHat className="w-4 h-4" />
                        </div>
                        <div>
                          <span className="font-display text-sm uppercase font-bold text-[#1C1917] block leading-tight">
                            Contact Kitchen &amp; Chef
                          </span>
                          <span className="text-[10px] font-mono text-black/50">Direct Studio Hotline</span>
                        </div>
                      </div>
                      <span className="text-xs font-mono font-bold text-[#4A2818] bg-amber-50 px-2 py-1 rounded-lg border border-amber-200">
                        90196 31104
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 pt-1">
                      <a
                        href="tel:9019631104"
                        className="py-2 px-3 rounded-xl bg-[#4A2818] hover:bg-[#2E1509] text-white font-mono text-xs font-bold uppercase transition-all flex items-center justify-center space-x-1.5 shadow-xs"
                      >
                        <Phone className="w-3.5 h-3.5" />
                        <span>Call Chef</span>
                      </a>
                      ...
                      <a
                        href={`https://wa.me/919019631104?text=${encodeURIComponent(`Hi Chef! I am inquiring about my Zafiroo Order #${trackingCodeOrToken}.`)}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="py-2 px-3 rounded-xl bg-[#25D366] hover:bg-[#1EBE5D] text-white font-mono text-xs font-bold uppercase transition-all flex items-center justify-center space-x-1.5 shadow-xs"
                      >
                        <span>WhatsApp</span>
                      </a>
                    </div>
                  ...
                  {currentOrder.deliveryMethod === 'delivery' && (
                    ...
                    >
                      <span>Share Live GPS Pin via WhatsApp</span>
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="w-full py-3 px-4 rounded-2xl bg-amber-50 hover:bg-amber-100/80 border border-amber-300/80 text-[#4A2818] font-mono text-xs font-bold uppercase flex items-center justify-center space-x-2 transition-all active:scale-95 shadow-xs cursor-pointer"
                  >
                  ...
                  <div className="pt-2 flex items-center justify-between gap-4 border-t border-cream-300">
                    ...
                    <button
                      onClick={() => setTrackingModalOpen(false)}
                      className="px-6 py-2.5 rounded-full bg-banhmi-dark hover:bg-espresso-900 text-white font-display text-sm uppercase tracking-wider font-bold transition-colors cursor-pointer"
                    >
                      Close
                    </button>
                  </div>
    ...
    {/* Bill Preview Modal for Customer */}
    <BillModal
      order={currentOrder}
